...
RSpec.describe Cliente, type: :model do
  describe 'validations' do
    ...
    it 'is not valid with duplicate identificacion' do
      create(:cliente, identificacion: '12345678-9')
  ...
  describe 'scopes' do
    let!(:cliente_activo) { create(:cliente, activo: true) }
    let!(:cliente_inactivo) { create(:cliente, activo: false) }

    it 'returns only active clients' do
      expect(Cliente.activos).to include(cliente_activo)
      expect(Cliente.activos).not_to include(cliente_inactivo)
    end
  end

  describe 'methods' do
    let(:cliente) { create(:cliente, nombre: 'Juan Perez', identificacion: '12345678-9') }
